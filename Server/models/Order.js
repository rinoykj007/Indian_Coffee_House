    ...
    if (this.isNew && !this.orderNumber) {
      ...
      while (attempts < maxAttempts) {
        try {
          ...
          const existingOrder = await this.constructor.findOne({ orderNumber: nextOrderNumber });
          ...
          if (!existingOrder) {
            ...
            this.orderNumber = nextOrderNumber + Math.floor(Math.random() * 100) + 1;
            const checkAgain = await this.constructor.findOne({ orderNumber: this.orderNumber });
            if (!checkAgain) {
              console.log('Generated order number with offset:', this.orderNumber);
              break;
            }
          }
          ...
          console.error(`Order number generation attempt ${attempts + 1} failed:`, error);
          attempts++;
        }
      }
