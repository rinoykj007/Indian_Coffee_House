...
router.get('/reports/daily', async (req, res) => {
  try {
    ...
    const todayRevenue = todayPayments.reduce((sum, payment) => sum + payment.totalAmount, 0);
...
router.post('/process', async (req, res) => {
  try {
    ...
    console.log('Payment processed:', savedPayment.paymentId);
...
router.get('/daily', async (req, res) => {
  try {
    ...
    const totalItems = payments.reduce((sum, payment) => sum + payment.itemsCount, 0);
    
    const paymentMethods = payments.reduce((acc, payment) => {
      acc[payment.paymentMethod] = (acc[payment.paymentMethod] || 0) + payment.totalAmount;
      return acc;
    }, {});
